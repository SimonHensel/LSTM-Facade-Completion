...
def multi_directional_md_rnn_while_loop(rnn_size, input_data, sh, dims=None, scope_n="layer1"):
    """
    Fragen:
    ...
    out1, state1 = multi_dimensional_rnn_while_loop(rnn_size, input_data, sh, dims, scope_n)
    ...
    out2, state2 = multi_dimensional_rnn_while_loop(rnn_size, input_data, sh, dims, "layer2")#DEACTIVATE FOR 2 DIRECTIONS
    ...
    out3, state3 = multi_dimensional_rnn_while_loop(rnn_size, input_data, sh, dims, "layer3")
    ...
    #print(input_data_3.shape)
    out4, state4 = multi_dimensional_rnn_while_loop(rnn_size, input_data, sh, dims, "layer4")#DEACTIVATE FOR 2 DIRECTIONS
    #prepare output
    ...
    CHANGE OUTPUT BACK
    '''
    out2 = tf.reverse(out2,[2])                                                #DEACTIVATE FOR 2 DIRECTIONS
    out2 = tf.transpose(out2,perm=[0, 2, 1, 3])                                #DEACTIVATE FOR 2 DIRECTIONS

    out3 = tf.reverse(out3,[2])
    out3 = tf.transpose(out3,perm=[0, 2, 1, 3])

    out3 = tf.reverse(out3,[2])
    out3 = tf.transpose(out3,perm=[0, 2, 1, 3])

    out4 = tf.reverse(out4,[2])                                                #DEACTIVATE FOR 2 DIRECTIONS
    out4 = tf.transpose(out4,perm=[0, 2, 1, 3])                                #DEACTIVATE FOR 2 DIRECTIONS
    ...
    output1 = slim.fully_connected(inputs=out1,
                                 num_outputs=1,
                                 activation_fn=tf.nn.sigmoid)
    output2 = slim.fully_connected(inputs=out2,                                #DEACTIVATE FOR 2 DIRECTIONS
                                 num_outputs=1,                                #DEACTIVATE FOR 2 DIRECTIONS
                                 activation_fn=tf.nn.sigmoid)                  #DEACTIVATE FOR 2 DIRECTIONS
    output3 = slim.fully_connected(inputs=out3,
                                 num_outputs=1,
                                 activation_fn=tf.nn.sigmoid)
    output4 = slim.fully_connected(inputs=out4,                                #DEACTIVATE FOR 2 DIRECTIONS
                                 num_outputs=1,                                #DEACTIVATE FOR 2 DIRECTIONS
                                 activation_fn=tf.nn.sigmoid)                  #DEACTIVATE FOR 2 DIRECTIONS

    output = tf.stack([tf.squeeze(output1,axis=3),tf.squeeze(output2,axis=3),tf.squeeze(output3,axis=3),tf.squeeze(output4,axis=3)],axis=3)                                                #DEACTIVATE FOR 2 DIRECTIONS
    print(output1)
    print(output2)
    print(output3)
    print(output4)
    """
    Hier Fehler
    Tensor("fully_connected/Sigmoid:0", shape=(1, 25, 25, 1), dtype=float32)
    Tensor("fully_connected_1/Sigmoid:0", shape=(1, 25, 25, 1), dtype=float32)

    """
    #output = tf.stack([tf.squeeze(output1,axis=3),tf.squeeze(output3,axis=3)],axis=3)
    #print("\n NEUER CODE\n")
    print(output)
