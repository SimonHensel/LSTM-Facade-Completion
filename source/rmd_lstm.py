...
def rotated_md_rnn_while_loop(rnn_size, input_data, sh, dims=None, scope_n="layer1"):
    """
    Fragen:
    ...
    #print(input_data_3.shape)
    out4, state4 = multi_dimensional_rnn_while_loop(rnn_size, input_data, sh, dims, "layer4")
    #prepare output
    ...
    out4 = tf.reverse(out4,[2])                                                
    out4 = tf.transpose(out4,perm=[0, 2, 1, 3])                               
    ...
    output1 = slim.fully_connected(inputs=out1,
                                 num_outputs=1,
                                 activation_fn=tf.nn.sigmoid)
    output2 = slim.fully_connected(inputs=out2,                                
                                 num_outputs=1,                                
                                 activation_fn=tf.nn.sigmoid)                  
    output3 = slim.fully_connected(inputs=out3,
                                 num_outputs=1,
                                 activation_fn=tf.nn.sigmoid)
    output4 = slim.fully_connected(inputs=out4,                                
                                 num_outputs=1,                                
                                 activation_fn=tf.nn.sigmoid)                  

    output = tf.stack([tf.squeeze(output1,axis=3),tf.squeeze(output2,axis=3),tf.squeeze(output3,axis=3),tf.squeeze(output4,axis=3)],axis=3)                                               
